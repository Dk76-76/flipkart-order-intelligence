...
def main():
    ...
    thresholds = [round(0.10 + i * 0.02, 2) for i in range(41)]
    ...
    if candidates.empty:
        raise RuntimeError("No threshold gives the required recall improvement.")
    ...
    results_df.to_csv(
        OUTPUT_DIR / "logistic_threshold_results.csv",
        index=False,
    )

    with open(
        OUTPUT_DIR / "logistic_threshold_summary.txt",
        "w",
        encoding="utf-8",
    ) as file:
        file.write("Logistic Regression Threshold Analysis\n")
        ...
        file.write(f"F1-optimal threshold: {best_f1['threshold']:.2f}\n")
        file.write(f"Operational threshold: {operational['threshold']:.2f}\n")
        file.write(f"Operational precision: {operational['precision']:.4f}\n")
        file.write(f"Operational recall: {operational['recall']:.4f}\n")
        file.write(f"Operational F1: {operational['f1']:.4f}\n")
        file.write(
    ...
    print(f"Recall improvement: {operational['recall_gain']:.2%}")
